...
class OnnxNetwork final : public Network {
 public:
  ...
  bool IsTRTEngineGood(std::filesystem::file_time_type start, int batch_size,
                       uint64_t hash, size_t onnx_model_size, int attempt, int optimize);
...
Ort::SessionOptions OnnxNetwork::GetOptions(int threads, int batch_size,
                                            uint64_t hash, int attempt, int optimize) {
...
OnnxNetwork::OnnxNetwork(const WeightsFile& file, const OptionsDict& opts,
  ...
  for (int step = 1; step <= steps_; step++) {
    ...
    session_.emplace_back(
        ...
        GetOptions(threads, batch_size_ * step, hash, attempt++, optimize));

    if (provider == OnnxProvider::TRT && (fp16_ || bf16_)) {
      if (!IsTRTEngineGood(start, batch_size_ * step, hash,
                           file.onnx_model().model().size(), attempt, optimize)) {
        if (attempt > 3) {
