...
HybridRatioMode EncodeHybridRatioMode(const std::string& mode_str) {
  ...
  if (mode_str == "thermal_annealing")
    return HybridRatioMode::THERMAL_ANNEALING;
  if (mode_str == "asymptotic_approach")
    return HybridRatioMode::ASYMPTOTIC_APPROACH;
...
const OptionId SearchParams::kTemperatureUtilityMaximumOffsetId{
...
void SearchParams::Populate(OptionsParser* options) {
  ...
  // START: ADDED FOR DYNAMIC HYBRID RATIO
  std::vector<std::string> hybrid_modes = {"static",
                                           "manual_schedule",
                                           "linear",
                                           "logarithmic",
                                           "power",
  ...
  options->Add<FloatOption>(kTemperatureVisitOffsetId, -100000000.0f,
                            100000000.0f) = 0.0f;
...
SearchParams::SearchParams(const OptionsDict& options)
    : options_(options),
      ...
      // START: ADDED FOR DYNAMIC HYBRID RATIO
      kHybridRatioMode(
          EncodeHybridRatioMode(options.Get<std::string>(kHybridRatioModeId))),
      kHybridRatioSchedule(ParseHybridRatioSchedule(
          options.Get<std::string>(kHybridRatioScheduleId)))
// END: ADDED FOR DYNAMIC HYBRID RATIO
{}
...
float SearchParams::GetDynamicHybridRatio(int node_count,
  ...
  if (mode == HybridRatioMode::MANUAL_SCHEDULE) {
    if (kHybridRatioSchedule.empty() ||
        node_count <= kHybridRatioSchedule.front().first) {
      return kHybridRatioSchedule.empty() ? 0.0f
                                          : kHybridRatioSchedule.front().second;
    }
  ...
  // Use the WL-adjusted scale instead of the raw scaling factor.
  const float p1 = GetHybridShapeParam1();
  const float p2 = GetHybridShapeParam2();

  // Progress is a value from 0.0 to 1.0+ representing how far we are through
  // the scaling phase. Uses adjusted sc_limit and adjusted scale.
  const float progress =
      (scale > 0) ? (static_cast<float>(node_count - sc_limit) / scale) : 1.0f;
  ...
  switch (mode) {
    ...
    case HybridRatioMode::LOGARITHMIC:
      ratio =
          min_r + (max_r - min_r) *
                      (std::log(clamped_progress * (2.71828f - 1.0f) + 1.0f));
      break;
    case HybridRatioMode::POWER:
      ratio = min_r + (max_r - min_r) *
                          std::pow(clamped_progress, p1);  // p1 is exponent
      break;
    ...
    case HybridRatioMode::SIGMOID:
      ratio = min_r + (max_r - min_r) /
                          (1.0f + std::exp(-12.0f * (clamped_progress - 0.5f)));
      break;
    case HybridRatioMode::EXPONENTIAL:
      ratio = min_r + (max_r - min_r) * (std::exp(clamped_progress) - 1.0f) /
                          (2.71828f - 1.0f);
      break;
    case HybridRatioMode::STEP_DECAY:
      if (p1 > 0) {  // p1 is number of steps
        ratio =
            max_r - (max_r - min_r) * std::floor(clamped_progress * p1) / p1;
      }
      break;
    case HybridRatioMode::INVERSE_SIGMOID:
      ratio =
          ...
          (max_r - min_r) *
              (1.0f -
               (1.0f / (1.0f + std::exp(-12.0f * (clamped_progress - 0.5f)))));
      break;
    case HybridRatioMode::STEPS:
      if (p1 > 0) {  // p1 is number of steps
        ratio =
            min_r + (max_r - min_r) * std::floor(clamped_progress * p1) / p1;
      }
      break;
    case HybridRatioMode::GAUSSIAN_PEAK: {  // p1=center [0,1], p2=width_sq
      float exponent =
          -std::pow(clamped_progress - p1, 2) / (2.0f * std::max(0.001f, p2));
    ...
    case HybridRatioMode::DOUBLE_PEAK: {  // p1=center1, p2=center2
      ...
      ratio =
          min_r + (max_r - min_r) * std::max(std::exp(exp1), std::exp(exp2));
      break;
    }
    case HybridRatioMode::SAWTOOTH_WAVE:
      if (p1 > 0) {  // p1 is period in nodes
        ratio =
            min_r +
            (max_r - min_r) *
                (std::fmod(static_cast<float>(node_count - sc_limit), p1) / p1);
      }
      break;
    case HybridRatioMode::OSCILLATING:
      if (p1 > 0) {  // p1 is period in nodes
        ...
        ratio = min_r +
                (max_r - min_r) *
                    (0.5f +
                     0.5f * std::sin(2.0f * pi * (node_count - sc_limit) / p1));
      }
      break;
    case HybridRatioMode::THERMAL_ANNEALING:
      ratio =
          (max_r * (1.0f - clamped_progress)) * Random::Get().GetFloat(1.0f) +
          (min_r + (max_r - min_r) * clamped_progress);
      break;
    ...
    case HybridRatioMode::PLATEAU:  // p1=mid_ratio, p2=width [0,1]
      if (clamped_progress < (0.5f - p2 / 2.0f)) {  // Ramp up
        float sub_progress =
            clamped_progress / std::max(0.001f, 0.5f - p2 / 2.0f);
    ...
    case HybridRatioMode::MULTI_TIMESCALE:
      if (p1 > 0 && p2 > 0) {  // p1=slow_period, p2=fast_period
        ...
        float slow_wave =
            0.6f *
            (0.5f + 0.5f * std::sin(2.0f * pi * (node_count - sc_limit) / p1));
        float fast_wave =
            0.4f *
            (0.5f + 0.5f * std::sin(2.0f * pi * (node_count - sc_limit) / p2));
