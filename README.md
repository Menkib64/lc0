...
You will need a [ROCm](https://rocm.docs.amd.com/) installation providing `hipcc` and hipBLAS. Enable the backend with `-Dhip=true`. The target GPU architecture is taken from `-Damd_gfx` (e.g. `-Damd_gfx=gfx90a`); if it is omitted it is autodetected with `rocm_agent_enumerator`, defaulting to `gfx90a`. Use `hip_libdirs` and `hip_include` if the ROCm libraries and headers are not in the default search path. lc0's HIP build links the kernels objects (built by `hipcc`) into a non-LTO host build, so pass `-Db_lto=false`.
